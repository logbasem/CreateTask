/* CREATE TASK REQUIREMENTS:/* CREATE TASK REQUIREMENTS:
    -INPUT
    -OUTPUT
    -PROCEDURES
...
The purpose of this program is to create an interactive choose-your-own-adventure fantasy story for the user, with a variety of possible paths and
endings.

CSS Styling and HTML Template from Web Dev Simplified on Youtube (https://youtu.be/R1S_NhKkvGA) */
...
"Love Wins", "The REAL Real Treasure", "So No Rock?", "Never Gonna-", "CURSES!!!"];
...
//options mapped to playAgain state
...
//options mapped to adventureReady state
function adventureReadyOptions(buttonNum) {
    if (buttonNum == 0) {
        state = fantasyStart;
        responseAndOptions("Huzzah! Let the adventure begin!", ["Your sword!", "Your wand!", "Nothing!"]);
    } else {
...
//options mapped to fantasyStart state
...
//options mapped to fantasyEnterDungeon state
...
//options mapped to fantasyLeftPath state
...
//options mapped to fantasyForwardPath state
...
//options mapped to fantasyOpenChest state
...
//options mapped to fantasyFindDoor state
...
//options mapped to fantasyDoorOpened state
...
//options mapped to fantasyInspectRock state
...
//options mapped to fantasyKickRock state
...
//options mapped to fantasyAttackMonster state
...
//options mapped to fantasyDazeMonster state
...
//options mapped to fantasyKeepFighting state
...
//options mapped to fantasyLootCamp state
...
//options mapped to fantasyGreetMonsters state
...
//options mapped to fantasyOfferTrade state
...
//options mapped to fantasyEnterKey state
...
//options mapped to fantasyDodgeClub state
...
//options mapped to fantasyAttackJimbor state
...
//option mapped to fantasyAssessJimbor state
...
//options mapped to fantasyDodgeForward state
...
//options mapped to fantasyDodgeBackward state
...
//options mapped to fantasyBothAssess state
...
//options mapped to fantasyJimborImplore state
...
//options mapped to fantasyBefriendJimbor state
...
//options mapped to fantasyAttemptUnlock state
function fantasyAttemptUnlockOptions(buttonNum) {
    if (buttonNum == 0) {
        if (rock) {
            //"True" Ending, probably hardest ending to achieve
            responseAndEnd(fantasyTreasure, "The REAL Real Treasure");
        } else {
